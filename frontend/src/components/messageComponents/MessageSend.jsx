...
const MessageSend = ({ chatId }) => {
	...
	return (
		<>
			...
			<form
				className="w-full flex items-center gap-2 h-[7vh] px-3 bg-black border-t-2 border-neon-green"
				style={{ boxShadow: "0 -2px 0 #39ff14" }}
				onSubmit={(e) => e.preventDefault()}
			>
				<label htmlFor="media" className="cursor-pointer flex-shrink-0">
					<FaFolderOpen
						title="Open File"
						size={18}
						className="text-neon-green/50 hover:text-neon-green transition-all"
				...
				<input
					type="text"
					className="retro-input h-[5vh] text-base"
				...
				<span className="flex justify-center items-center flex-shrink-0">
					{newMessage?.trim() && !isSendLoading && (
						<button
							className="w-9 h-9 flex items-center justify-center pixel-border hover:bg-neon-green/10 transition-all"
					...
					{isSendLoading && (
						<button className="w-9 h-9 flex items-center justify-center pixel-border">
							<LuLoader
								title="loading..."
								fontSize={16}
								className="animate-spin text-neon-green"
							/>
						</button>
					)}
				</span>
